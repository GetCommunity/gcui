...
import { Show, children, createEffect, createSignal, onMount } from 'solid-js';
...
export default function Pagination(props: PaginationProps) {
  ...
  onMount(() => setPageCount());
  createEffect(() => setPageCount());
  const itemStyles = `tabular-nums transition-colors ${pageItemSizeKey[size]} ${pageItemEdgeCurve[curve]} ${pageColorKey[color][pageStyleType]}`;
  return (
    <KBPagination
      ...
      itemComponent={(props) => (
        <KBPagination.Item page={props.page} class={`pagination__item ${itemStyles}`}>
          {props.page}
        </KBPagination.Item>
      )}
      ellipsisComponent={() => (
        <KBPagination.Ellipsis
          class={`pagination__ellipsis ${pageItemSizeKey[size]} ${pageItemEdgeCurve[curve]}`}
        >
          {ellipsisText()}
        </KBPagination.Ellipsis>
      )}
