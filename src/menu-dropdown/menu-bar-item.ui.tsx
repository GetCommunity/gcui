...
export const MenubarItem: Component<MenuItemProps> = (props) => {
  ...
  return (
    <KBMenubar.Item
      class={`menubar__item ${menuItemColor[color()][styleType()]} ${menuItemSizeKey[size()]} ${menuItemEdgeCurve[curve()]}`}
    >
      {props.textValue}{' '}
      <Show when={props.rightSlot}>
        <div class='menubar__item-right-slot'>{props.rightSlot}</div>
      </Show>
    </KBMenubar.Item>
  );
};
