...
const Collapsable: ParentComponent<CollapsableProps> = (props) => {
  ...
  return (
    <KBCollapsible class={`collapsible group ${itemCurve[curve]} ${itemColor[color]}`}>
      <KBCollapsible.Trigger
        class={`collapsible__trigger ${triggerCurve[curve]} ${triggerColor[color][styleType]} ${itemSize[size]}`}
      >
        <span class={`collapsible__trigger-text`}>{label()}</span>
        <span
          class={`collapsible__trigger-icon ${itemIconSize[size]} ui-group-expanded:rotate-180 rotate-0 transition-transform`}
        >
          <KeyboardArrowDownIcon aria-hidden />
        </span>
      </KBCollapsible.Trigger>
      <KBCollapsible.Content
        class={`collapsible__content animate-collapsable-slide-up ui-expanded:animate-collapsable-slide-down ${contentColor[color]}`}
      >
